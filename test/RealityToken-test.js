...
const RealityToken = require('../build/contracts').RealityToken;

describe('ERC777 RealityToken', function() {
  let realityToken;
  let server;
  let web3;
  let accounts;
  ...
  describe('Deploy', function() {
    it('should deploy the RealityToken', async function() {
      realityToken = await RealityToken.new(
        web3, 'Reference Token', 'XRT', web3.utils.toWei('0.01')
      );
      assert.ok(realityToken.$address);
    });

    after(function() { util.setTokenContract(realityToken); });
  });

  describe('Attributes', async function() {
    beforeEach(function() {
      Object.assign(this, { accounts, realityToken, web3, erc820Registry });
    });
